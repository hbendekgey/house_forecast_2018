...
We notice that the predictions seem to be systematically biased in favor of incumbents, having overestimated incumbent performance for every single election since 2006. The size of this bias is on average 7.5 seats. If we make this adjustment, calculate standard error, and produce a distribution of seats, we see: 

<<include=FALSE>>=
lm(actual ~ offset(1 *pred), data=sxpred) %>% summary()
sigma <- (lm(actual ~ offset(1 *pred), data=sxpred) %>% summary())$sigma
mu <- (interval$fit[1] - 59)/2 -7.545
@
...
Adding the X-part of the Structure-X model pulls the seat change further towards Democrats and reduces the standard erorr, causing a great deal of confidence. The maximum error of the adjusted Structure-X model in the past 6 elections is 10 seats.
...
Thus we predict Republicans will lose 52 seats, and estimate a 99.7\% chance of Democrats taking the house.
