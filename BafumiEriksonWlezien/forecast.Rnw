...
compare_model <- filter(model, year != 2014)

fit <- lm(vote ~ genpoll + president_party, data=compare_model)
@
...
My model claims:
$$\textnormal{National Vote \%}=-0.57 + 0.47\textnormal{Generic Poll \%} -2.14 \textnormal{PresidentialParty} + e_j$$

The size of the effects are almost identical. However, mainly due to the intercept, my model will produce an estimate for national swing that is about a half percentage point smaller than Bafumi's. 

I did not include 2014 data in this model so I could compare to Bafumi et al's regression in their 2014 paper. Now I can include that data and see if my estimates change:

<<updated regression>>=
updated_fit <- lm(vote ~ genpoll + president_party, data=model)
summary(updated_fit)
@

Fortuntely, they do not. We wouldn't want a single election to have a huge influence on the regression. Now let's run this model on data from 2018 to predict the national vote. First, we find the general congressional ballot

<<2018 generic ballot, include=FALSE>>=
generic_topline <- read_csv("~/generic_topline.csv") %>%
  filter(subgroup == "Voters") %>%
  mutate(date = as.Date(modeldate, "%m/%d/%Y")) %>%
  filter(date > as.Date("5/9/2018", "%m/%d/%Y"), date < as.Date("7/9/2018", "%m/%d/%Y")) %>%
  mutate(dem_share = dem_estimate/(dem_estimate+rep_estimate))
genpoll <- mean(generic_topline$dem_share) * 100 - 50
params18 <- data.frame(genpoll=genpoll, president_party=-1)
@
...
<<prediction interval>>=
interval <- predict.lm(updated_fit, params18, interval = "prediction")
interval
-0.14 + 0.53 * genpoll +2.04 #Bafumi et al point estimate
@
...
qt(0.975, df=15) # t-cutoff of 2.12
c((3.367+0.5591)/2.131,(7.293-3.367)/2.131) #SE for t-distribution is 1.842
@
...
expswing <- 4.493 # if I use mine, I will get 3.926
sdswing <- 1.842
dfswing <- 15
@
...
\item Count Pennsylvanian incumbents as incumbents? Currently I'm not because of redistricting.
\end{itemize}
