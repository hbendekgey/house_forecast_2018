...
house14 <- read_csv("~/fec-election-results/fec_tidy.csv") %>% 
  ...
  group_by(name) %>%
  summarise(district = district[1], party =getparty(party), 
...
Recall that a large value indicates a stronger correlation between how blue a district is and how few people vote. In this case, contrary to Bafumi et al's findings, 2016 was the worst year for Democratic turnout, and 2012 and 2014 were not substantially different. In fact, 2014 is the year where this effect is the smallest. (I note at this point that Bafumi et al claim the discrepency in 2012 was 0.49, contrary to my 0.55)
...
We see that these 327 districts shifted by an average of -3.48 points. Ultimately, this is the value we are interested in, and it is incredibly close to the national vote swing of -3.55. 
...
We have 41 open seats. They vary from our prediction of them (using the template above):
...
Thus we say that districts with open seats will vary with a standard deviation of 6.2 percentage points from their expected center given voting history and national swing.
...
myopenvar #6.186 compared to 6.197. Basically the exact same thing.
@
...
This is interesting. If we run our own linear model on 2014's prediction from 2012, we see that a lot more weight it put on how the incumbent preformed in the last election(0.85 in my model vs 0.63 in Bafumi et al's), and much less weight is put on how it voted for President (0.15 in my model compared to 0.46 in Bafumi et al's). These are highly correlated, so it is unsurprising that this would shift year-to-year. Picking which model to use in 2018 is largely ideological: how much, in your opinion, is the GOP the party of Trump? TO what degree is it still the party of the incumbents? 

<<myincvar, include=FALSE>>=
myincvar <- var(sum_inc$residuals) %>% sqrt()
myincvar #3.787 compared to 4.1.
@
...
openint # -2.4. This is the "base intercept"
...
\item Currently we concede all races in 2018 that went uncontested in 2016. Is this a potential danger? Is there a race that might have gone from uncontested to competitive in 2 years? If so, what is a more sophisticated model we could use, given that we don't have a precedent for how the district votes?
...
\item Talk about NY 13 in 2012, and CA 30, CA 44, IA 03, LA 03, and OH 16 in 2014 (multiple incumbents)
